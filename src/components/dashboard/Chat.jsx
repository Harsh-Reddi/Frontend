...
const socket = io('https://backend-n789.onrender.com')
...
const Chat = () => {
    ...
    const {my_friends, fd_messages, currentFd, successMessage} = useSelector(state => state.chat)
            ...
            <div className='w-full flex'>
                <div className='w-[230px]'>
                    ...
                    <div className='w-full flex flex-col text-slate-600 py-4 h-[400px] pr-3'>
                        {
                            my_friends.map((f,i) => <Link to={`/dashboard/chat/${f.fdId}`} className={`flex gap-2 justify-start items-center pl-2 py-[5px]`} >
                            <div className='w-[30px] h-[30px] rounded-full relative'>
                            {
                                ...
                                <img className='w-[30px] h-[30px] rounded-full' src={f.image} alt="" />
                            </div>
                            <span>{f.name}</span>
                            </Link> )
                        }
                        ...
                        currentFd ? <div className='w-full h-full'>
                            <div className='flex justify-start gap-3 items-center text-slate-600 text-xl h-[50px]'>
                                <div className='w-[30px] h-[30px] rounded-full relative'>
                                {
                                    ...
                                    <img className='w-[30px] h-[30px] rounded-full' src={currentFd.image} alt="" />
                                </div>
                                        ...
                                        if(currentFd?.fdId !== m.receiverId){
                                            return (
                                                <div ref={scrollRef} key={i} className='w-full flex gap-2 justify-start items-center text-[14px]'>
                                                    <img className='w-[30px] h-[30px] ' src="https://frontend-lilac-nine.vercel.app/images/user.png" alt="" />
